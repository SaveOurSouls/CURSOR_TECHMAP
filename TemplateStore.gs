...
/**
 * Перепаковывает _TC_STORE так чтобы живые блоки шли подряд от строки 1,
 * и обновляет storeRow в каталоге.
 */
function compactifyStore_(catalog) {
  ...
  runWithSheetVisible_(storeSheet, () => {
    ...
    if (!live.length) {
      ...
      if (maxRow > 0) storeSheet.deleteRows(1, maxRow);
      return;
    }
    ...
    for (const item of live) {
      if (item.storeRow !== cursor) { alreadyCompact = false; break; }
      cursor += item.height;
    }
