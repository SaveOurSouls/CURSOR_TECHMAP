...
const ASSEMBLY_GEN = {
  ...
  // Видимый плейсхолдер в пустых заполняемых ячейках шаблона: подсказка для
  // ручной сборки «сюда вносятся данные». Генератор считает его пустым и
  // перезаписывает (см. isPlaceholderOrEmpty_).
  fillPlaceholder: '‹…›',
};
